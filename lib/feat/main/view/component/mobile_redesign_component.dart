...
class _MobileModePill extends StatelessWidget {
  const _MobileModePill({
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          _ModePillTab(label: '이용자', active: !isOperator, onTap: onUserTap),
          _ModePillTab(label: '운용자', active: isOperator, onTap: onOperatorTap),
        ],
      ),
    );
  }
...
class _ModePillTab extends StatelessWidget {
  const _ModePillTab({
    required this.label,
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: active ? DC.canvas : Colors.transparent,
          borderRadius: BorderRadius.circular(DC.rxPill),
          boxShadow:
              active
                  ? <BoxShadow>[
  ...
  Widget build(BuildContext context) {
    ...
    return CustomScrollView(
      ...
      slivers: <Widget>[
        ...
        SliverToBoxAdapter(child: _LiveOperatorsBannerSection(store: store)),
        SliverToBoxAdapter(
          child: KeyedSubtree(
        ...
        SliverToBoxAdapter(
          child: _OperatorCtaBand(onTap: () => context.push('/pilot/register')),
        ),
          ...
          children: <Widget>[
            ...
            Row(
              children: <Widget>[
                ...
                Builder(
                  builder: (_) {
                    final hasProposedPrice =
                        quote.price.isNotEmpty && quote.price != '-';
                    final displayPrice =
                        hasProposedPrice ? quote.price : quote.budgetOption;
                    return Text(
                      displayPrice,
                      style: TextStyle(
                        fontFamily: 'Pretendard',
...
class _MobileQuoteEditSheetState extends ConsumerState<_MobileQuoteEditSheet> {
  ...
  static const _budgetChips = <String>['0~30만원', '30~50만원', '50~100만원', '협의'];
  ...
  void initState() {
    ...
    final saved = q.budgetRange;
    if (_budgetChips.contains(saved)) {
      // 예산 칩 옵션이면 칩 선택, 금액 필드 비움
      _budget = saved;
      _amountController = TextEditingController();
    } else if (saved.isNotEmpty) {
      // 직접 입력한 금액이면 금액 필드에 채우고 '협의' 칩 선택
      _budget = '협의';
      _amountController = TextEditingController(text: saved);
    } else {
      _budget = '0~30만원';
      _amountController = TextEditingController();
    }
